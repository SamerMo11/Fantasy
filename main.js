    ...
    let budget = 50;
    let selectedPlayers = new Set(); // تخزين اللاعبين المختارين
    ...
    function displayPlayers(event) {
        ...
        chars()
            .filter(player => player.key === key && !selectedPlayers.has(player.name)) // تصفية اللاعبين المختارين
            .forEach(player => {
    ...
    function selectPlayer(player) {
        ...
        lastClickedButton.setAttribute("data-player", player.name); // تخزين اسم اللاعب في الزر
    
        selectedPlayers.add(player.name); // إضافة اللاعب إلى القائمة
    ...
    function removePlayer(button) {
        ...
        selectedPlayers.delete(playerName); // إزالة اللاعب من القائمة ليظهر مجددًا
    }
